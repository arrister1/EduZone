  ...
  Widget build(BuildContext context) {
    return Scaffold(body: LayoutBuilder(
      ...
      if (constraints.maxWidth <= 600) {
        return LangGrid(gridCount: 2);
      } else if (constraints.maxWidth <= 1200) {
        return LangGrid(gridCount: 4);
      } else {
        return LangGrid(gridCount: 6);
      }
    }));
  }
}

class LangGrid extends StatelessWidget {
  final int gridCount;

  const LangGrid({Key? key, required this.gridCount}) : super(key: key);
      ...
      body: Container(
        child: Padding(
            ...
            child: GridView.count(
              ...
              crossAxisSpacing: 5,
              mainAxisSpacing: 20,
              children: courseLang.map((course) {
                return InkWell(
                  onTap: () {
